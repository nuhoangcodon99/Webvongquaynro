        ...
        <div class="col-lg-6 offset-lg-3">
            ...
            <?php } else { ?>
                ...
                if ($_SERVER["REQUEST_METHOD"] == "POST") {
                    ...
                    if ($statement_check->rowCount() == 0) {
                        ...
                        if ($row["ban"] == 1) {
                            ...
                            // Cập nhật tiền
                            $sql_update = "UPDATE account SET thoi_vang = thoi_vang + :thoi_vang WHERE username = :username";
                            $statement_update = $conn->prepare($sql_update);
                            $statement_update->bindParam(':thoi_vang', $vnd, PDO::PARAM_INT);
                            $statement_update->bindParam(':username', $username, PDO::PARAM_STR);

                            if ($statement_update->execute()) {
                                $_alert = '<div class="alert alert-success">Cộng xu thành công!</div>';
                            } else {
                ...
                <form method="POST">
                    ...
                    <div class="mb-3">
                        <label> Xu:</label>
                        <input type="thoi_vang" class="form-control" name="thoi_vang" id="thoi_vang" placeholder="Nhập xu cần cộng"
                            required autocomplete="thoi_vang">
                    </div>
