...
<div class="container pt-5 pb-5">
    <div class="row">
        <div class="col-lg-6 offset-lg-3">
            <br>
            <br>
            <h4>Cộng Tổng Nạp - Máy Chủ 1</h4><br>
            <?php if ($_admin != 1) { ?>
            ...
            <?php } else { ?>
                ...
                if ($_SERVER["REQUEST_METHOD"] == "POST") {
                    ...
                    if ($statement_check->rowCount() == 0) {
                        ...
                        if ($row["ban"] == 1) {
                            ...
                            // Cập nhật tiền
                            $sql_update = "UPDATE account SET tongnap = tongnap + :tongnap WHERE username = :username";
                            $statement_update = $conn->prepare($sql_update);
                            $statement_update->bindParam(':tongnap', $vnd, PDO::PARAM_INT);
                            $statement_update->bindParam(':username', $username, PDO::PARAM_STR);

                            if ($statement_update->execute()) {
                                $_alert = '<div class="alert alert-success">Cộng tổng nạp thành công!</div>';
                            } else {
                ...
                <form method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="tongnap" class="form-control" name="tongnap" id="tongnap" placeholder="Nhập tổng nạp cần cộng"
                            required autocomplete="tongnap">
                    </div>
